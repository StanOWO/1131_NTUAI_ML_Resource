...
def classify_result(x, y, classifier, fg_color=("orange", "blue"), bg_color=("red", "green"), title="", font=""):
    ...
    background_grid = pd.DataFrame(np.column_stack([X_background.ravel(), Y_background.ravel()]))
    Target_predict = classifier.predict(background_grid).reshape(X_background.shape)
    plt.contourf(X_background, Y_background, Target_predict, alpha=0.75, cmap=ListedColormap(list(bg_color)))
    ...
    for y_real_index, y_real in enumerate(np.unique(y)):
        row_selector = y.reshape(-1) # y.ndim =2, we need 1D array to select rows of X
        plt.scatter(x[row_selector == y_real, 0], x[row_selector == y_real, 1], color=fg_color[y_real_index], label=f"Class {y_real}")
